@if ($selected_application)
    <div class="py-3 px-5 space-y-5 border border-gray-300 rounded-md h-full bg-gray-100 flex flex-col relative" x-data="chattingFunction" x-init="initialize">
        ...
        <div class="h-full overflow-y-auto space-y-3 mt-auto relative">
                    ...
                    <template x-if="message.receiver_id == '{{ Auth::user()->id }}'">
                        <div class="flex items-start gap-2.5">
                            ...
                            <div class="flex flex-col w-full max-w-[320px] leading-1.5 p-4 border-gray-200 bg-white rounded-e-xl rounded-es-xl">
                                ...
                                <template x-if="message.is_deleted">
                                    <span class="text-sm font-normal pt-2.5 text-gray-900 italic">The Message has been deleted</span>
                                </template>
                            </div>
        ...
        Alpine.data('chattingFunction', () => ({
            ...
            initialize(){
                ...
                if(jobId) {
                    ...
                    Echo.private("private-chat.{{ Auth::user()->id }}")
            ...
            sendMessage(application, jobId, senderId, receiverId) {
            ...
            sendEditedMessage(messageId) {
                ...
                axios.post("/send-edited-message", {
                    ...
                    let id = this.messages.findIndex(message => message.id == res.data.id)
                    this.messages[id] = res.data;
                }).catch((err) => {
                    console.log(err,err.response)
                }).finally(() => {
                    this.sending = false
