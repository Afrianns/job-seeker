        ...
        <div class="py-3 px-5 space-y-5">
            ...
            <div class="flex justify-between items-center">
                ...
                @if (Auth::check() && !Auth::user()->is_recruiter)
                    <span data-modal-target="report-modal" data-modal-toggle="report-modal" type="button" class="cursor-pointer text-red-500 hover:text-red-700 hover:underline font-medium rounded-lg text-sm">Report this job!</span>
                @endif
            </div>
                    ...
                    @if (Auth::check())
                        ...
                        @else
                            @if ($job->application()->exists() && Auth::user()->application()->exists())
                                <a href="{{ route('my-application', ['id' => $job->application->first()->id]) }}" class="focus:outline-none text-white bg-purple-700 hover:bg-purple-800 focus:ring-4 focus:ring-purple-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 cursor-pointer">Check application status</a>
                            @else
                                <button data-modal-target="static-modal" data-modal-toggle="static-modal"  type="button" class="focus:outline-none text-white bg-purple-700 hover:bg-purple-800 focus:ring-4 focus:ring-purple-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2">Apply</button>
                            @endif
                        @endif
                    @endif
                </div>
                @if (Auth::user()->is_recruiter && Auth::user()->company->id == $job->company->id)
                    <button type="button" data-modal-target="popup-modal" data-modal-toggle="popup-modal" class="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2">Delete</button>
                @endif
            </div>
                ...
                <div id="report-modal" data-modal-backdrop="static"  tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center bg-purple-50/50 items-center w-full h-full">
                    <div class="relative p-4 w-full max-w-md max-h-full">
                        ...
                        <div class="relative bg-white rounded-lg shadow-sm">
                            ...
                            <!-- Modal body -->
                            <form class="p-4 md:p-5" action="{{ route('reporting-job') }}" method="post">
                                @csrf
                                ...
                                <button type="submit" class="text-white inline-flex items-center bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center  -700 -800">
                                    Send Report
                                </button>
                            </form>
                        </div>
