...
<x-layout>
    ...
    <div class="max-w-[1140px] mx-auto mt-5 flex flex-col justify-center items-center min-h-[calc(100vh-150px)]" x-data="registerFunction">
        <h2 class="text-center mb-5 text-2xl font-medium">Recruiter Register</h2>
        <form action="/auth/recruiter-register" method="POST" class="py-6 px-5 bg-white border border-gray-300 rounded-md w-full max-w-[500px] space-y-5 mx-auto">
            @csrf
            <div class="mb-5">
            ...
            <div class="mb-5">
                ...
                <input type="text" name="company_name" id="company_name" @class(['bg-gray-50 text-gray-900 text-sm rounded-lg block w-full p-2.5 border', 'focus:ring-red-500 focus:border-red-500 border-2 border-red-300' => $errors->has('company_name'), 'focus:ring-blue-500 focus:border-blue-500 border-gray-300' => !$errors->has('company_name')]) placeholder="Your company title" required />
                @error('company_name')
                    <p class="bg-red-500 text-red-50 py-2 px-5 rounded mt-3">{{ $message }}</p>
                @enderror
            </div>
            <div class="mb-5">
                ...
                <input type="email" name="company_email" id="company_email" @class(['bg-gray-50 text-gray-900 text-sm rounded-lg block w-full p-2.5 border', 'focus:ring-red-500 focus:border-red-500 border-2 border-red-300' => $errors->has('company_email'), 'focus:ring-blue-500 focus:border-blue-500 border-gray-300' => !$errors->has('company_email')]) placeholder="Your company email" required />
                @error('company_email')
                <p class="bg-red-500 text-red-50 py-2 px-5 rounded mt-3">{{ $message }}</p>
                @enderror
            </div>
